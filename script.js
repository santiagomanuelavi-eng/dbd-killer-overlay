...
function renderKillers(){
    ...
    killers.forEach(killer => {
        ...
        const selectedIndex = selectedOrder.findIndex(k => k.name === killer.name);
...
function selectKiller(killer){
    ...
    const exists = selectedOrder.find(k => k.name === killer.name);

    if(exists){
        selectedOrder = selectedOrder.filter(k => k.name !== killer.name);
    }else{
